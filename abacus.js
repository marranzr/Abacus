var canvas = document.getElementById('canvas'),
	...
	BEAD_WIDTH = 48,
	BEAD_HEIGHT = 30,
	BEAD_COLOR = 'rgba(255,250,245,1)',
	BEAD_STROKE = 'rgba(245,240,235,1)',
	BEAD_STROKE = 'black',
...
// Constructors
 ...
 Bead.prototype = {
 	getPoints: function() {
 		var points = [],
 			center = this.evalPosition();
 		points.push(new Point(center.x - BEAD_WIDTH/2, center.y)); //   .
 		points.push(new Point(center.x + BEAD_WIDTH/2, center.y)); //   ____
 		points.push(new Point(center.x + BEAD_WIDTH/4, center.y - BEAD_HEIGHT/2)); // ____\
 	...
 	evalPosition: function() { // returns the central point of the bead; 
 		var x = MARGIN + this.rod*DISTANCE_RODS,
 			y = undefined;
...
function drawRods() {
	...
	for ( var i = 0, x = MARGIN + DISTANCE_RODS; i < numberOfRods; ++i, x += DISTANCE_RODS) {
		...
		context.stroke();
		drawDot(i, x, MARGIN + HEAVEN);
	}
	
	context.restore();
}

function drawDot(num, x, y) {
	context.save();
	var middle = Math.floor(numberOfRods / 2); // Dot in this and this +- 3
	if ((num - middle) % 3 === 0) {
		context.lineWidth = 1;
